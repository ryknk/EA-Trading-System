...
* [ ] US30_HISTの実SymbolのVolume Maxを250から100へ再適用する（`ApplyCfdSymbolSpec`をUS30_HISTへ`InpApply=true`で実行。Digits設定を含むため、実行前にtickとバー履歴（.hcc）をバックアップし、MT5を停止して実行する。承認volumeの最大は0.7で、既存の結果には影響しない）
* [ ] US30の2021-05-31より前（最小取引単位1.00だった期間）のバックテストの扱いを決める（Custom Symbolは全期間で0.1のため、当時は発注できない数量が含まれる。初見資産の動作確認用途のため、結果の解釈に注意する）
