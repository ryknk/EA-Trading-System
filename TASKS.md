...
**追加分析（2026-09-17実施、ユーザー依頼、4銘柄合計でのWalk Forward再測定と要因分解）**: 過去ログから現行設定一式を正確に再構成できなかったため、現行の確定パラメータをそのまま使いWalk Forward期間（2020-2024年次Fold、4銘柄）を再測定した（CaseFile`mt5/test-config/cases/walk-forward-2020-2024-current-config.json`、`StrategyTester-Generic-H1.ini`使用）。

* **4銘柄合計の年次期待値は、単一銘柄の記録以上に明確な単調悪化トレンドを示した**: 2020年+400.6円/トレード→2021年+551.2円→**2022年+581.9円（ピーク）**→2023年+103.7円（急落）→**2024年-151.9円（赤字転落）**→Final Holdout(2025-2026)**-160.2円**。2024年に赤字転落した水準が、Final Holdoutでもほぼそのまま（横ばい〜微悪化で）継続している。4銘柄いずれも2024年前後に悪化しており、特定通貨ペア固有の問題ではない。
* **市場レジーム分布（TrendUp/TrendDown比率）とADX強度（entry_adx平均44〜45）は2020〜2025年でほぼ一定**。Entry判定の"入力側"（強いトレンドと判定される頻度）は劣化していない。
* **BUY/SELLの優位性は年ごとに不規則に反転**（2020 SELL優位→2021-2023 BUY優位→2024 SELL優位→Final Holdout BUY優位・SELL壊滅（-52,641円））。系統的な方向バイアスの劣化というより、方向性判定（HTFバイアス）自体に元々系統的な優位性が乏しく、その年の相場にたまたま乗れたかどうかで結果が決まっている可能性がある。
* **PULLBACKパターンの勝率が2022年以降急落**（2020-2021年36-37%→2022-2023年11-14%→2024年20%→Final Holdout 23.3%）。少数派（全体の1〜2割）だが、BREAKOUTより先行して質的変化が見えた。
* **最も本質的な発見: 含み益到達トレード（mfe>0、全体の93〜99%）のうち、「Peak確定後の逆行幅」（`post_peak_mae_r`）は年を通じてほぼ一定（-0.845〜-0.898R）だが、「含み益ピークの大きさ」（`mfe_r`平均）は0.983→0.945→0.931→0.917→0.889→**0.813（Final Holdout）**と緩やかに単調減少している**。逆行幅（分子）が一定で山の高さ（分母）が縮んでいるため、giveback比率が算術的に押し上げられ、Final Holdoutで完全反転率が最も高くなった（71.8%、次点は2020年70.5%）。これは「トレンドが崩れた後にどこまで戻されるか」ではなく、**「一度トレンドに乗った後にどこまで伸びるか」自体が年々小さくなっている**ことを示す、最も具体的な構造変化の証拠である。

**追加分析2（2026-09-17実施、ユーザー依頼、IS期間まで遡ったmfe_r全期間推移確認）**: 上記の「2020-2024で単調減少」という所見が、恒久的な構造変化か循環的な現象かを切り分けるため、In-Sample期間（2017-09〜2019-12、4銘柄、2020年分は既存のWalk Forward再測定データを流用）を現行設定一式で追加測定した（CaseFile`mt5/test-config/cases/is-period-2017-2019-current-config.json`）。**この結果、「2022年ピークからの単調減少」という直前の解釈は訂正が必要と判明した。**

| 期間 | 該当数 | mfe_r平均 | post_peak_mae_r | 期待値/トレード |
|---|---:|---:|---:|---:|
| 2017(9-12月) | 35 | 1.120 | -0.911 | +1,172.6円 |
| **2018** | 105 | **0.725** | -0.923 | **-910.2円** |
| 2019 | 68 | 0.903 | -0.894 | +102.3円 |
| 2020 | 105 | 0.983 | -0.889 | +400.6円 |
| 2021 | 113 | 0.945 | -0.895 | +551.2円 |
| 2022 | 93 | 0.931 | -0.845 | +581.9円（ピーク） |
| 2023 | 96 | 0.917 | -0.898 | +103.7円 |
| 2024 | 93 | 0.889 | -0.855 | -151.9円 |
| FH(25-26) | 174 | 0.813 | -0.849 | -160.2円 |

**2018年に、Final Holdout（0.813）を下回る深い落ち込み（mfe_r 0.725、期待値-910.2円）が既に発生し、翌2019年から2022年にかけて回復していた。** つまりmfe_rの低下は2022年以降に初めて起きた現象ではなく、2018年にも一度発生し、その時は1年で底を打って回復している。`post_peak_mae_r`（逆行幅）は全期間を通じて-0.845〜-0.923の狭いレンジで安定しており、これは循環・構造変化のどちらの局面でも変わらないこの戦略の恒常的な特性である。

**修正した結論**: 「恒久的な構造変化」と断定するのは2018年の前例を踏まえると時期尚早である。ただし、2018年の落ち込みが1年で回復したのに対し、**今回（2023年〜Final Holdoutまで）の下降は既に4年目に入っており、回復の兆しがまだ見えていない**点は、単純な循環と楽観視するには継続期間が長い。「2018年型の一時的な落ち込みか、今回は本当に恒久的な構造変化か」は、現時点のデータだけでは判別できず、実際に今後1〜2年の推移を観測する以外に確実な切り分け方法がない。

**結論**: Final Holdoutでの成績悪化は、Fold1-5で調整した特定パラメータ（EarlyAdverseExit等）の過学習という局所的な問題ではない。「トレンドの持続力・伸びしろの縮小」（含み益ピークの縮小、逆行幅は不変）という構造は明確に観測されたが、これが恒久的な市場構造の変化か、2018年型の循環的な落ち込みが長引いているだけかは追加分析だけでは判別できない未解決の論点として残る。いずれにせよ単純なパラメータ再調整で解決する問題ではなく、本番移行判定は引き続き**NO-GO**とする。詳細は`docs/production-readiness-report.md`を参照。

**残存リスク・未確認事項**: Final Holdoutは4銘柄・単一期間のみの一度きりの評価であり、統計的サンプル数は限定的（181トレード）。Walk Forward・IS期間再測定は既存の監査ログの再集計で完結しており追加のFinal Holdout消費は発生していないが、「トレンドの伸びしろ縮小」が恒久的か循環的かは未解決。この仮説自体は本プロジェクトの4銘柄（すべて円クロス・ユーロクロス中心）の範囲でのみ確認されたものであり、他の資産クラス・より長期のヒストリーでの裏付けは未確認。Final Holdout自体は既に消費済みのため、追加のパラメータ変更を行う場合は新しいFinal Holdout期間の確保が必要（`DECISIONS.md` DEC-024参照）。

**追加分析3（2026-09-17実施、ユーザー依頼、他資産クラスでのmfe_r検証と、その過程で発見した既存不具合の原因調査）**: 4銘柄がすべて円・ユーロクロス（相関が高い可能性）であることを踏まえ、異なる資産クラス（JP225=日本株指数、US30=米国株指数、XAUUSD=金、いずれもFold1-5への適合には一切使っていない初見資産、production判定の根拠にはしない）でmfe_r低下パターンの再現性を確認する追加測定を行った（CaseFile`mt5/test-config/cases/cross-asset-mfe-check.json`）。

**調査過程で発見した不具合と原因確定**: 当初、これら3銘柄の実データ最古日（JP225/US30=2020-04、XAUUSD=2022-06）に近い期間で実行したところ、`SIGNAL_ERROR code=MARKET_DATA_UNAVAILABLE`がテスト期間全体を通じて恒常的に発生し、取引が1件も発生しない異常が生じた。一時的なデバッグ出力（`TrendFollowingStrategy.mqh::Evaluate()`、調査後に削除済み・恒久変更なし）で原因を特定した結果、**D1（トレンド判定用）の200期間EMAインジケーターハンドル（`m_d1_slow_handle`）だけが、実行開始から終了まで`BarsCalculated()=-1`のまま一度も回復しなかった**（H4/H1系の指標は正常）。これは新しい不具合ではなく、**DEC-025で既に文書化されていた既知の制約と同一の原因**であることが判明した（`docs/backtesting.md`57行目「Tester起動時点でD1/H4インジケーターの計算に必要な事前バッファが不足していたことが判明し、テスト実行中に指標が後から回復することもないと確認した」）。USDJPY_HISTでは実データ最古日から約9-10か月のバッファ確保（IS開始日2017-09-01）で回避済みだったが、XAUUSD_HIST等では検証時の開始日が実データ最古日から9.2か月しかなく、同じ境界制約に抵触していた。**対応（コード変更不要）**: 開始日を実データ最古日から12か月確保するよう補正（JP225/US30は2021-04-01、XAUUSDは2023-07-01）したところ、3銘柄とも`SIGNAL_ERROR`が解消し正常動作を確認した。EA側のロジックバグではなくMT5 Strategy Tester（またはCustom Symbol実装）側の制約であり、DEC-025の運用ルール（検証開始日は実データ最古日から十分なバッファを確保する）が全Custom Symbol共通で有効であることが再確認された。

**追加分析3の続き: JP225/US30/XAUUSDで取引が成立しない残り2層の原因確定と対応（2026-09-19、ユーザー依頼、`DECISIONS.md` DEC-033）**: 上記のD1 EMAウォームアップ制約を回避した後も、取引が1件も成立しなかった。原因は**EA側の不具合ではなく、Custom Symbolの仕様が未設定（初期値のまま）**であることを、Strategy Tester内の仕様取得（新規`DiagnoseSymbolSpecEA`）で確定した。接続先Broker（OANDA-Japan MT5 Demo）は全51銘柄がFXのみで、これら3銘柄の実Symbolが存在せず、複製元なしで作成した結果、Digits=4・Point=0.0001・契約サイズ=100000・Volume Min/Step/Max=1e-8/1e-8/1e-7、JP225/US30の利益通貨は銘柄名の切れ端（`25_`・`0_H`）となっていた。

* **`SPREAD_TOO_WIDE`**: 実スプレッドの超過ではなくPoint定義の誤り（実スプレッドはJP225約6円、XAUUSD約0.3〜5ドル程度で妥当）。
* **`TICK_VALUE_UNAVAILABLE`（JP225/US30）**: 利益通貨が不正で`OrderCalcProfit`が損益0を返し、フォールバックのTickValueも0。US30も再現確認済み（11件全件）。
* **`RISK_STATE_UNAVAILABLE`（XAUUSD）**: reasonは`INVALID_EXPOSURE_INPUT`（`RiskManager.mqh:176`経由、`ExposureGuard.mqh:51`）。契約サイズ100000のまま1ロット損失が約3.1億円となりロットがVolume Maxに張り付き、`PositionSizer::VolumeDigits(1e-8)`が0桁を返して`NormalizeDouble`後にvolume=0のまま`Calculate()`がtrueを返したためと推定（コード解析による推定。volume=0だったことは監査ログで確認）。正常な仕様（Volume Step 0.01等）では発生しないため、EA側は変更していない。

**対応（Custom Symbol側の修正、EA本体・4銘柄の設定は無変更）**: OANDA公式ページ（Web取得結果、原ページとの照合は未実施）に基づく暫定仕様を`mt5/Tools/ApplyCfdSymbolSpec.mq5`で設定する。追加作業で判明した事項は次のとおり。(1) Digitsの変更でMT5がバー履歴（.hcc）を消去する（tickは残る）ため、`mt5/Tools/RebuildBarsFromTicks.mq5`で保持tickからバーを再生成する必要がある。(2) 証拠金は、CalcModeがCFDINDEX/FUTURESだと`SYMBOL_MARGIN_INITIAL`（固定額）基準で計算され0のままだったため、CFDモード＋証拠金率（JP225/US30 10%、XAUUSD 5%）を使う（実験用複製銘柄で4モードを比較して確定）。(3) JP225_HISTは適用・検証済み（Locally Tested）。2021-04〜12でRISK_DECISION承認10件（TICK_VALUE_UNAVAILABLE・SPREAD_TOO_WIDE・RISK_STATE_UNAVAILABLEはゼロ）、承認時`required_margin`約13.5万円、保有中の証拠金維持率約1,140%（`results/backtests/20260919-131018-JP225_HIST-H1`）。US30_HIST（2021-04〜12、承認19件、`required_margin`約18.5万円、`results/backtests/20260919-132411-US30_HIST-H1`）・XAUUSD_HIST（2023-07〜12、承認10件、`required_margin`約5.5万円、`results/backtests/20260919-132447-XAUUSD_HIST-H1`）も同様に適用・動作確認済み（バー再生成は全月・失敗0）。**これは「EAが取引できるか」の確認であり、成績・パラメータ・採用判断・Final Holdout代替の根拠には使わない。**

* [x] JP225の仕様値（Volume Step 1か0.1か等）とUS30の契約サイズ／数量の意味を、OANDA公式原ページで照合した（2026-09-19、`DECISIONS.md` DEC-033「原ページとの照合結果」。JP225のStep 1、US30の契約サイズ1・Step 0.1は一致。**US30のVolume Maxが250でなく100**と判明し、`ApplyCfdSymbolSpec.mq5`を訂正。XAUUSDのStepは原ページに記載なし）
* [x] US30_HISTの実SymbolのVolume Maxを250から100へ再適用した（2026-09-19、`DECISIONS.md` DEC-033「US30_HISTの再適用」。変更はこの1項目のみ。バー履歴・tickはバックアップとSHA-256一致、再実行のRISK_DECISION 33件が完全一致）。元の記述: US30_HISTの実SymbolのVolume Maxを250から100へ再適用する（`ApplyCfdSymbolSpec`をUS30_HISTへ`InpApply=true`で実行。Digits設定を含むため、実行前にtickとバー履歴（.hcc）をバックアップし、MT5を停止して実行する。承認volumeの最大は0.7で、既存の結果には影響しない）
* [x] US30の2021-05-31より前（最小取引単位1.00だった期間）のバックテストの扱いを決めた（2026-09-19、`DECISIONS.md` DEC-033。Custom Symbolは変更せず、US30・US100の動作確認は2021-06-01以降で行い、それ以前を含む結果は「当時は発注できなかった数量を含む」と注記する。US30 2021-06-01〜12-31で承認16件・採否が同一であることを確認済み）。元の記述: US30の2021-05-31より前（最小取引単位1.00だった期間）のバックテストの扱いを決める（Custom Symbolは全期間で0.1のため、当時は発注できない数量が含まれる。初見資産の動作確認用途のため、結果の解釈に注意する）
* [x] **【修復済み・残差解消、2026-09-19】`RebuildBarsFromTicks`（`ApplyCfdSymbolSpec`の`InpRebuildFromMonth`を含む）で直近月のtickが欠落し、その後のバー履歴の誤りで残差が生じた件。** 経緯: (1) バックアップ（書き戻し前）と現行の`.tkc`サイズの比較でtick欠落が判明（JP225_HIST 2026-02〜08は4.7〜12.8MB→65〜128KB、US30_HIST 2026-05〜08、XAUUSD_HIST 2026-08）。原因は、terminalの`CopyTicksRange`/`CopyTicks`が直近月で実データの約1%（128件ブロック単位）しか返す仕様（Digitsを変えていないUSDJPY_HIST・EURUSD_HIST・US100_HIST等の2026年8月でも同じで、私の変更とは無関係の元からの挙動）を知らず、その読み取り結果を`CustomTicksReplace`で書き戻したこと。(2) tickは書き戻し前のバックアップで3銘柄とも復元した（`.tkc`のSHA-256一致）。(3) その後、Tester上のtick数が書き戻し前と僅かに異なる残差（JP225 +21,466、US30 +11,329、XAUUSD +429、Testerログの「real ticks absent」＝合成tick）が出た。**根本原因**: 再生成したバー履歴（`.hcc`）が約1%のtickだけから作られた誤りのバーで、tickのない分足にもバーが存在し、Testerが実tickなしの分足として合成tickで補ったため。未使用のUS500_HISTでの対照実験で確定した（Digitsのみ変更＋元のバー履歴＝差異なし、再生成後のバー履歴＋復元tick＝欠如91分・+2,772 tick、元のバー履歴に戻す＝差異なし）。Digits変更自体は原因ではない。**修復**: US30_HIST・XAUUSD_HISTはDigits変更前のバー履歴のバックアップ（`history-*-before`）を戻した（US500の実験で、Digits変更後に元のバー履歴を戻しても動作することを確認済み）。Testerのtick数はSep 17と完全一致（US30 62,782,089、XAUUSD 165,511,616）、候補も全件一致（US30 102/102、XAUUSD 112/112、差異0）。JP225_HISTは元のバー履歴のバックアップがないため、影響月（2026-02〜08）のバーを`BarsFileTool`の削除モードで削除し、履歴の終端に目印バー2本だけ置いてTesterに全tickから生成させ（`ExportM1BarsEA`）、ティックのある分足のバーだけを取り込み直した（202,939本）。Testerのtick数は34,419,937（Sep 17は34,419,941、差4は目印バーの2分）、欠如分の記録なし、候補97/97が全項目（ATR・ADX・スプレッド含む）で一致。**確認済みの事実・推定**: 正常な月の出力バーとterminalのバーがOHLC完全一致（約5〜6万本ずつ）で、出力方法の妥当性を確認。影響月でも、バーの有無にかかわらず一部の分足（例: JP225 2026-02-02 05:08のtick 5件）のtickをTesterが受け取っていない（tick総数がSep 17と同等）ため、直近月のAPI可視ブロック（約0.65%）はTesterのtick集合に元から含まれていなかった可能性が高い（推定・未検証。書き戻し前から存在した性質で、私の変更が作った差ではない）。**運用**: 3銘柄の2026年分の結果は、上記の元からの性質を踏まえた参考値とする。2026年より前は書き戻し前と全項目で一致し、影響なし。
* [x] 非FX資産で価格水準・スプレッド水準が大きく変わった場合の`InpMaxSpreadPoints`の妥当性を確認した（2026-09-19、`DECISIONS.md` DEC-034「追加確認」。US30は全期間で問題なし。JP225は2024年以降、XAUUSDは2025年以降、Broker側のスプレッド水準が倍増して超過率10〜43%になる。価格比の方式でも解消しないため、閾値は変更せず、結果報告に`SPREAD_TOO_WIDE`割合を併記する運用とした）
* [x] 非FX資産用テンプレートの`InpMaxSpreadPoints`の決め方を決定した（2026-09-19、`DECISIONS.md` DEC-034。FX4銘柄の30 pointsの価格比0.0227%を基準価格へ適用し、JP225 110・US30 110・XAUUSD 770 pointsとして事前に固定、結果を見て調整しない）。元の記述: 非FX資産用テンプレートの`InpMaxSpreadPoints`の決め方を決定する（Pointが正しくなっても既定値30では全候補が拒否される。現テンプレートの500000〜60000は緩和値のまま）
* [x] `PositionSizer::VolumeDigits`の防御を実施した（2026-09-19、DEC-034。正規化後のvolumeが0以下なら`SIZE_BELOW_MIN`で拒否。Final Holdout 4銘柄の再実行で結果・全335件のRISK_DECISIONが完全一致）。元の記述: `PositionSizer::VolumeDigits`が極小Volume Stepで0桁を返し、volume=0のままtrueを返しうる点をEA側で防御するか判断する（4銘柄の売買ロジックへ影響しないことを示す必要がある。現時点では変更していない）
* [x] Strategy Testerの`Leverage=10`が反映されない件を調査した（2026-09-19、読み取り＋診断EAのみ）。**原因**: iniの`Leverage=<数値のみ>`は無視され、Testerは端末設定（`terminal.ini`の`[Tester] Leverage=`、実測で100）に保存された値を使う。`Leverage=1:25`・`Leverage=1:100`のように`1:N`形式で書くと反映される（診断EAで確認: `1:25`→25、`1:100`→100、`10`/`25`/`50`/`100`/`500`→保存値のまま）。**既存結果への影響**: IS・Walk Forward・Final Holdout・今回の3銘柄テストを含む全実行が、意図した1:10でも実口座の1:25（診断スクリプトで実測したデモ口座のレバレッジ）でもなく、保存値の1:100で実行されていた（全実行の`.htm`レポートは1:100、`tester.ini`は`Leverage=10`）。IS/WF/Final Holdoutの922件の承認判断と10,477件の口座スナップショットで、証拠金を10倍（1:10相当）に換算して試算すると、証拠金維持率がガード150%を下回るのは1スナップショットのみ（最悪138.9%）で、証拠金不足系の拒否は元の結果に含まれていない。4倍（1:25相当）では最悪約347%、最大`required_margin`約27万円で、ガードに抵触しない見込み（いずれもスケーリングによる試算で、再実行による実測ではない）。したがって既存4銘柄の結果が変わる可能性は小さいが、ゼロとは確認していない。**再実行による実測確認（2026-09-19実施）**: iniの`Leverage`だけを`1:25`に変え、他は同一のテンプレート・期間で再実行した。Final Holdout（4銘柄、基準`results/backtests/20260916-223649-cases`、再実行`20260919-133712-cases`）は、4銘柄とも取引数・純損益・PFが完全一致（合計181トレード・-28,991円、リスク判定件数も承認183・POSITION_LIMIT 136・SPREAD_TOO_WIDE 16で同一）。Walk Forward（20ケース、基準`20260917-214524-cases`、再実行`20260919-134534-cases`）も、20ケースすべてで取引数・純損益が一致（合計520トレード・+157,169円）。最低の証拠金維持率は、Final Holdoutで1,381%→345%、Walk Forwardで2,659%→665%に下がったが、ガード（150%）には抵触せず、拒否の発生もなかった。IS期間（`20260917-222112-cases`）は再実行していない（過去の試算では1:25でもガードに抵触しない見込み）。よって**既存4銘柄の結果は、1:100と1:25で変わらないことを、Final HoldoutとWalk Forwardについて実測で確認した**。実口座（1:25）に近い条件でも証拠金ガードは効かず、これらの結果は引き続き有効。
**テンプレート修正（2026-09-19実施）**: `mt5/test-config/StrategyTester*.ini`のうち`Leverage=10`だった77件（追跡済み74件・未追跡3件）を、Leverage行のみ`Leverage=1:25`へ変更した（他の行は無変更）。修正後のテンプレートで実行し、レポートが1:25になることを確認済み（`20260919-140820-JP225_HIST-H1`）。今後の実行は実口座のレバレッジ（1:25）で走る。**注意**: 過去の実行記録（`run-metadata`のtemplate_sha256）は修正前のテンプレートのハッシュで、修正後のテンプレートとは一致しない。過去結果の実行条件は「テンプレートは`Leverage=10`だが、実際は保存値の1:100」だった。
* [x] 既存4銘柄の主要ケース（Final Holdout・Walk Forward）を`Leverage=1:25`で再実行し、結果が変わらないことを実測で確認した（2026-09-19、確認目的のみでパラメータ調整には不使用）
* [x] 実験用複製銘柄`JP225_XMCFD`・`JP225_XMLEV`・`JP225_XMFUT`・`JP225_XMIDX`を削除する（2026-09-19完了、`MarginExperimentSymbols`の`InpDelete=true`、`EXP_DELETE_COMPLETED`を確認）

**追加分析3の結果: 3資産（JP225/US30/XAUUSD）でのmfe_r（2026-09-19実施、ユーザー依頼、DEC-033/034適用後の仕様・Leverage 1:25・`InpMaxSpreadPoints`=110/110/770の事前固定値、13ケース、`results/backtests/20260919-165902-cases`）**: 全13ケースで`SYSTEM_ERROR`は0件、取引が成立した。初見資産であり、パラメータ調整・採用判断・production判定には使わない（確認目的のみ）。

| 期間（ケースにより開始日が異なる） | 3資産のトレード数 | TP到達率 | 期待値/トレード | mfe_r平均 | post_peak_mae_r | `SPREAD_TOO_WIDE`拒否率 | （参考）FX4銘柄のmfe_r / 期待値 |
|---|---:|---:|---:|---:|---:|---:|---|
| 2021（JP225/US30、2021-04〜） | 29 | 20.7% | +474円 | 0.868 | -0.814 | 3.4% | 0.945 / +551円 |
| 2022 | 53 | 18.9% | -299円 | 0.845 | -0.891 | 2.2% | 0.931 / +582円 |
| 2023（XAUUSDは2023-07〜） | 61 | 16.4% | -697円 | 0.779 | -0.914 | 1.8% | 0.917 / +104円 |
| 2024 | 96 | 17.7% | -308円 | 0.734 | -0.796 | 4.7% | 0.889 / -152円 |
| 2025-01〜2026-08 | 125 | 24.0% | +418円 | 0.818 | -0.728 | **21.9%**（JP225 19.6%・XAUUSD **42.9%**・US30 1.0%） | 0.813 / -160円 |

* **2021→2024のmfe_r低下は3資産でも見える**（0.868→0.734）。ただし**2025-2026は0.818へ反発**し、期待値も黒字（+418円）になった。FX4銘柄が2024年の赤字水準（-152円）から2025-2026もほぼ横ばい（-160円）だった点とは異なる。
* **水準・銘柄差が大きい**: 全期間のmfe_rはJP225 0.893・XAUUSD 0.843・US30 0.693、期待値はJP225 +12円・XAUUSD +515円・US30 -392円で、資産ごとにばらつく。`post_peak_mae_r`は全期間で-0.61〜-0.98のレンジ。
* **DEC-034の運用ルールに従い**、`SPREAD_TOO_WIDE`拒否が2025-2026のXAUUSD（42.9%）・JP225（19.6%）で結果を左右する期間として扱う。この2ケースの成績（特にXAUUSD 2025-2026の期待値+1,190円）は、拒否による候補の選別で偏っている可能性があり、他期間と単純比較しない。
* **解釈（暫定）**: 「トレンドの伸びしろ縮小が広い市場構造変化か、FX4銘柄（円・ユーロクロス）固有の現象か」の切り分けとしては、**2021-2024の低下は共通するが、2025-2026の反発はFX4銘柄と異なる**という部分的な結果にとどまり、FX固有と断定も、共通と断定もできない。2018年型の一時的な落ち込みだったという仮説（`docs/production-readiness-report.md` 7.5節）を弱く支持する材料ではあるが、確定的ではない。
* **限界**: (1) 期間ごとのトレード数が10〜50件、期間別の合算でも29〜125件と少なく、差（例: 2025-2026の0.818とFX 0.813）に統計的な有意性の検定は行っていない。(2) `mfe_r`はTP（2R）・建値ストップ・EarlyAdverseExitで保有が打ち切られる範囲内の最大含み益で、保有できた期間に依存する（全比較で設定は同一）。(3) 資産ごとの価格トレンド（JP225は約2.9万→6万、XAUUSDは約1,730→4,590）の影響を分離していない。(4) US30の2021-05-31以前は当時の最小取引単位が1.00で、Custom Symbolの0.1とは異なる（DEC-033）。(5) 3資産の投入tickは直近月にAPI制約の欠落があり修復済み（DEC-033）だが、全期間一致の確認は一部のみ。**未確認**: 他の資産（US100・US500・US2000、他通貨ペア）、より長期のヒストリー。

**追加分析3の続き: US100/US500/US2000のCustom Symbol仕様の確認と修正（2026-09-19、ユーザー依頼、`DECISIONS.md` DEC-035）**: 投入済み（2020-04〜2026-09）で仕様適用の記録がなかった3銘柄を確認した結果、**3銘柄とも仕様は初期値のまま**（Digits 4、契約サイズ100000、Volume 1e-8、CalcMode 0、通貨は文字化け）で、現行EAは2021-04〜12で取引0件（候補は全件`SPREAD_TOO_WIDE`）だった。JP225/US30/XAUUSDと同じ原因（EA側の不具合ではない）。

* [x] 3銘柄の現在の仕様と、現行EAでの拒否理由を確認した（2026-09-19、読み取りのみ、`20260919-192241-cases`）
* [x] 3銘柄をOANDA公式ページの原文で照合した（DEC-035「OANDA公式ページとの照合」。呼値・通貨・証拠金率（10%）・契約サイズ（1）・最小取引単位・刻み・最大取引数量を記録。原ページに記載のないスプレッド・スワップ・過去時点の仕様、投入tickの由来は**未確認**）
* [x] `ApplyCfdSymbolSpec.mq5`へ3銘柄を追加し、仕様を設定した（DEC-035。tick・.hccをバックアップ、tickはSHA-256一致、Digits変更で消えた.hccはバックアップから復元してSHA-256一致。`RebuildBarsFromTicks`は不使用）
* [x] `InpMaxSpreadPoints`をDEC-034の方式で事前固定した（US100 60、US500 20、US2000 540。2025-01〜2026-08の候補の価格中央値から。US500は10 points丸めで実質約1.3倍）。専用テンプレート3件を作成した
* [x] 各銘柄で短期間のバックテストを実行し、取引が成立することを確認した（US100 2021-06〜12: 14件、US500 2021: 34件、US2000 2021: 17件。動作確認のみで、成績は評価・採用判断に使わない）。仕様適用前後で候補（timestamp・entry_price・ATR）は3銘柄とも完全一致
* [x] 4銘柄（Final Holdout）へ影響がないことを確認した（適用後の再実行で、181トレードの結果と全335件のRISK_DECISIONが適用前と完全一致、`20260919-201501-cases`）
* [x] **US100・US500・US2000の全期間のスプレッド分布の確認（2026-09-26実施、ユーザー依頼、`DECISIONS.md` DEC-034「追加確認（続き）」）。** US100・US500は固定値（60・20 points）が実勢スプレッドに対して十分緩く、`SPREAD_TOO_WIDE`拒否は無視できる水準（`over_fixed`全期間0.0〜0.4%）。**US2000は固定値540に対し`over_fixed`が全期間で8〜14%、`over_ratio`は8〜60%に達し、JP225・XAUUSDと同様にスプレッドフィルターの拒否が結果を左右する銘柄と判明した。** 既存の6資産mfe_r検証結果（本節「6資産への見直し」）のUS2000の解釈には、この事実を踏まえた注意が必要。
* [x] **3銘柄の直近月のTesterのtick数と全期間の候補一致を確認した（2026-09-26実施、ユーザー依頼）。** `VerifyCustomSymbolTicks.mq5`でCopyTicksRangeベースのtick数を直近月（2026年6-9月）で確認したところ、8月・9月に急激な減少が見られた（US100: 8月84,480件・9月0件、US500: 8月10,367件・9月0件、US2000: 8月18,944件・9月0件）。この期間を含む短期間バックテスト（2026.08.01〜2026.09.21）を3銘柄で実行したところ、**いずれも「100%リアルティック」と報告された。** これはJP225・US30・XAUUSD（DEC-033）で確認済みの結論と一致する: CopyTicksRangeの制約（terminalのtick読み取りAPIが直近月で実データの一部しか返さない）はスクリプトからの外部読み取りに限定され、Strategy Tester内部エンジンの候補生成・バックテスト精度には影響しない。
* [x] **投入tickの由来を確認した（2026-09-26実施、ユーザー依頼）。** Git履歴・ファイルシステムのタイムスタンプの両方から投入元CSVの手がかりは見つからず（`bases\Custom\ticks\`内のファイル作成日時は今回セッション内のDigits変更・再構築時のもので、最初の投入時点は特定不可能）、**投入元CSVファイル自体の特定は断念した。** 代わりに、既存のスプレッド分布データ（月次平均価格、65か月分、2020-04〜2025-08）とYahoo Finance実指数終値を比較する間接検証を行った。

  | 銘柄 | 対応指数 | 比率（平均±標準偏差） | 範囲 |
  |---|---|---|---|
  | US100 | Nasdaq100（^NDX） | 0.9920 ± 0.0337 | 0.9284〜1.0816 |
  | US500 | S&P500（^GSPC） | 0.9920 ± 0.0267 | 0.9392〜1.0688 |
  | US2000 | Russell2000（^RUT） | 0.9942 ± 0.0303 | 0.9338〜1.0619 |

  3銘柄すべてで65か月にわたり体系的なズレなく高い一致度（比率0.99前後、標準偏差3%程度、月内平均vs月末値の差・CFDスプレッドの影響を考慮すれば妥当な範囲）を確認し、投入tickが実際にそれぞれの指数に対応するCFD価格であることを間接的に強く裏付けた。**投入元の記録（誰が・いつ・どのツールで投入したか）は失われたままだが、データの信頼性自体は価格水準の系統的照合により確認できた。**

**追加分析3の結果（続き）: 米国株指数3資産（US100/US500/US2000）を加えた6資産での見直し（2026-09-19実施、ユーザー依頼、DEC-035の仕様・`InpMaxSpreadPoints`=60/20/540の事前固定値・Leverage 1:25、15ケース、`results/backtests/20260919-213601-cases`、CaseFile`mt5/test-config/cases/cross-asset-mfe-check-us-indices.json`）**: 全15ケースで`SYSTEM_ERROR`は0件、取引が成立した（動作確認としてDEC-035の記録と整合）。初見資産であり、パラメータ調整・採用判断・production判定・Final Holdoutの代替には使わない。US100の2021年のみ、最小取引単位の変更（2021-05-31）を避けて2021-06-01開始とした。

| 期間 | 米国株指数3資産 トレード数 | TP到達率 | 期待値 | mfe_r | 6資産合算 mfe_r | 6資産合算 期待値 | 6資産 `SPREAD_TOO_WIDE`拒否率 | 6資産 `SIZE_BELOW_MIN`拒否率 | （参考）FX4銘柄 mfe_r / 期待値 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 2021 | 65 | 16.9% | -592円 | 0.663 | 0.729 | -263円 | 1.1% | 0.0% | 0.945 / +551円 |
| 2022 | 69 | 14.5% | -440円 | 0.711 | 0.770 | -379円 | 1.8% | 6.5% | 0.931 / +582円 |
| 2023 | 88 | 18.2% | -400円 | 0.755 | 0.764 | -522円 | 0.8% | 0.4% | 0.917 / +104円 |
| 2024 | 103 | 16.5% | -675円 | 0.660 | 0.696 | -498円 | 3.2% | 1.2% | 0.889 / -152円 |
| 2025-01〜2026-08 | 163 | 17.2% | -277円 | 0.668 | 0.731 | +25円 | 11.9% | 10.2% | 0.813 / -160円 |

* **米国株指数3資産には、時間的なトレンドが見られない**: mfe_rは0.663→0.711→0.755→0.660→0.668、期待値は全期間で負（-277〜-675円）。FX4銘柄のような2021→2024の単調低下も、前回の3資産（JP225/US30/XAUUSD）で見えた2025-2026の反発も見られない。**6資産合算のmfe_rは0.729→0.770→0.764→0.696→0.731とほぼ一定**で、FX4銘柄の低下（0.945→0.813）は6資産では再現しない。
* **水準は資産クラスで大きく異なる**: 全期間のmfe_rはJP225 0.893・XAUUSD 0.843・US100 0.776・US30 0.693・US500 0.655・US2000 0.616、期待値はJP225 +12円・XAUUSD +515円・US100 -92円・US500 -355円・US30 -392円・US2000 -999円。
* **解釈上の重大な制約（床効果）**: 米国株指数の3資産は、調査期間の初年（2021年）から既に期待値が負・TP到達率が損益分岐（33.3%）を大きく下回る（US2000 2021年はTP到達率0%）。**この戦略はこれらの資産では元からエッジがない**ため、FX4銘柄で観測された「エッジが失われていく変化」を検出できない。したがって「6資産でmfe_rの低下が再現しない」ことは、**FX固有である可能性を示唆する一方、エッジのない資産を混ぜたことによる希釈でも説明できる**。「市場全体の構造変化」を支持する証拠は得られなかったが、「FX固有」とも断定できない。
* **【訂正・見落とし】`SIZE_BELOW_MIN`の拒否**: 直前の3資産（JP225/US30/XAUUSD）の集計で、拒否理由の内訳（`POSITION_LIMIT`以外の「その他」）を確認せず、`SIZE_BELOW_MIN`（最小Volumeでもリスク予算を超えるための拒否）の発生を報告・記録していなかった。今回の再集計で判明した割合（全期間、候補に対する拒否率）: US500 14.5%（50/345、2022年24.4%・2025-2026年29.8%）、XAUUSD 9.5%（18/189、2025-2026年16.1%）、US30 3.6%（10/276）、US2000 2.1%（5/234）、US100 0.3%（1/309）、JP225 0%。**拒否された候補のSL距離の中央値は、承認された候補の約2〜3倍**（例: US500 2025-2026年 43.8 vs 20.1、XAUUSD 2025-2026年 38.3 vs 18.0）で、値幅の大きい（高ボラティリティの）局面の候補が選別的に除外されている。高ボラティリティ局面はトレンドの伸びしろが大きい可能性があり、mfe_rを過小評価する方向の偏りがありうる（方向は未検証）。これは正当なリスク拒否であり、EAの不具合ではない。**以後、非FX資産の結果には`SPREAD_TOO_WIDE`に加えて`SIZE_BELOW_MIN`の割合も併記する**（DEC-034の運用ルールへの追加。DECISIONS.mdへの反映は未実施）。DEC-035の「`SIZE_BELOW_MIN`は0件」は2021年（US100は2021-06〜12）の確認範囲の記述であり、この期間については正しい。
* **限界**: (1) 米国株指数3資産はUS30と同じ米国株指数で、独立した観測とは言えない。(2) ケースごとのトレード数は14〜67件と少なく、有意性の検定は行っていない。(3) 資産の価格トレンド（JP225・US100・US500の2025-2026年の上昇、XAUUSDの上昇）との交絡を分離していない。(4) 拒否（`SPREAD_TOO_WIDE`・`SIZE_BELOW_MIN`）による候補の選別が、特に2025-2026年（`SPREAD_TOO_WIDE` 11.9%、`SIZE_BELOW_MIN` 10.2%）で結果を歪めうる。(5) 未確認: FX同士でも円・ユーロを含まない通貨ペア（tick未投入のため未実施）、より長期のヒストリー、米国株指数以外の非FX資産。**(6) 【2026-09-26追記】US2000は全期間で`SPREAD_TOO_WIDE`の`over_fixed`拒否率が8〜14%（`DECISIONS.md` DEC-034「追加確認（続き）」）に達しており、上表のUS2000の期待値（-999円）・mfe_r（0.616、全資産中最低）は、この拒否の影響を強く受けている可能性がある。US2000固有の低い成績（元からエッジがない、という上記の「床効果」の解釈）自体を否定するものではないが、その内訳にスプレッド拒否による選別バイアスが混在している点に注意。**

**追加分析3の結果（続き）: FXメジャーペア（AUDUSD/USDCAD/NZDUSD）での見直し（2026-09-22〜23実施、ユーザー依頼。上記(5)の限界「FX同士でも円・ユーロを含まない通貨ペアは未確認」への対応）**: FX4銘柄（USDJPY/EURJPY/EURUSD/GBPJPY）がすべて円・ユーロクロスであることを踏まえ、円・ユーロを含まない主要FXペア（AUDUSD・USDCAD・NZDUSD）でmfe_r低下パターンの再現性を確認した。tickは新規実装のDukascopy取得・MT5変換パイプライン（`docs/tick-data-pipeline.md`、`DECISIONS.md` DEC-036〜040）で取得（2020-01-01〜2026-08-31、D1 200EMAウォームアップ確保のため2020年分は検証に使わずウォームアップ専用とした）。3銘柄ともOANDA-Japan MT5に実銘柄が存在しCustom Symbol仕様は実銘柄から複製されるため、JP225/US30/XAUUSD等で必要だったCustom Symbol仕様の事後修正（DEC-033/034）は不要で、`StrategyTester-Generic-H1.ini`（FX4銘柄と同じEA既定値）をそのまま使用した。CaseFile`mt5/test-config/cases/cross-asset-mfe-check-fx-pairs.json`（15ケース、`results/backtests/20260923-225220-cases`）。全15ケースで`SIGNAL_ERROR`・`SPREAD_TOO_WIDE`等の異常はなく取引が成立した。初見資産であり、パラメータ調整・採用判断・production判定・Final Holdoutの代替には使わない。

| 年 | 3銘柄合算 トレード数 | 勝率 | 期待値 | mfe_r | post_peak_mae_r | （参考）FX4銘柄 mfe_r / 期待値 |
|---|---:|---:|---:|---:|---:|---|
| 2021 | 70 | 22.9% | -435円 | 0.795 | -0.897 | 0.945 / +551円 |
| 2022 | 79 | 24.1% | -654円 | 0.749 | -0.875 | 0.931 / +582円 |
| 2023 | 68 | 20.6% | -951円 | 0.789 | -0.998 | 0.917 / +104円 |
| 2024 | 83 | 32.5% | +582円 | 0.849 | -0.754 | 0.889 / -152円 |
| 2025-01〜2026-08 | 113 | 23.9% | -764円 | 0.750 | -0.951 | 0.813 / -160円 |
| **全期間合算** | **413** | **24.9%** | **-447円** | **0.784** | **-0.895** | — |

銘柄別・全期間平均のmfe_rはAUDUSD 0.722・USDCAD 0.770・NZDUSD 0.863。

* **FX4銘柄のような単調低下が見られない**: mfe_rは0.795→0.749→0.789→0.849→0.750と上下に変動し、FX4銘柄の一貫した右肩下がり（0.945→0.813）は再現しない。
* **開始水準がFX4銘柄より一貫して低い**: FX4銘柄は2021年時点でmfe_r 0.945だったのに対し、3ペアは2021年時点で既に0.795しかなく、そもそも「高い水準から低下する」という前提が成立していない。
* **期待値はほぼ一貫して赤字基調**: 5期間中4期間が赤字（2024年のみ黒字+582円）。FX4銘柄のような「2020-2022年黒字→2023年以降赤字」という明確な転換点ではなく、継続的にエッジが薄い可能性がある。
* **解釈上の制約（床効果、米国株指数6資産の分析と同じ構図）**: 3ペアは調査期間を通じて開始水準が低く赤字基調であるため、FX4銘柄で観測された「エッジが失われていく変化」を検出できる土台になっていない可能性がある。したがって「3ペアでFX4銘柄の低下トレンドが再現しない」ことは、**円・ユーロクロス固有である可能性を示唆する一方、これらの資産にそもそもエッジが薄いことによる希釈でも説明できる**。「市場全体の構造変化」「FX全般に共通する変化」のいずれの仮説も支持する決定的な証拠は得られなかった。
* **限界**: (1) ケースごとのトレード数が13〜50件、全期間合算でも413件と少なく、有意性の検定は行っていない。(2) 資産の価格トレンド（AUDUSD/NZDUSDの豪州・NZ金利サイクル、USDCADの原油連動性）との交絡を分離していない。(3) tickデータソースがFX4銘柄（OANDA提供）と異なる（Dukascopy提供）ため、スプレッド・tick密度の違いが結果に影響している可能性がある（EURUSDでの相関係数0.999998の一致確認は別途実施済みだが、AUDUSD/USDCAD/NZDUSD自体でのOANDA版との突き合わせは未実施）。(4) 米国株指数6資産の分析と同様、他の候補拒否理由（`SIZE_BELOW_MIN`等）の内訳は今回集計していない。

**追加分析3の結果（続き）: FXメジャーペアをIS期間相当まで拡張＋GBPUSD新規追加（2026-09-24実施、ユーザー依頼）**: 上記3ペア検証（2021-2026のみ）に対するユーザー指摘「IS期間を2017-09より前にすることは検証の質を上げるか」を踏まえ、確定パラメータのまま検証範囲を広げるconfirmatory測定として、(1) tickを既存FX4銘柄と同じ2016-09（DEC-023のUSDJPY実データ最古日基準）まで遡って取得し、AUDUSD/USDCAD/NZDUSDの2017-2020年分を追加、(2) 円・ユーロを含まない主要ペアとしてGBPUSDを新規に全期間（2016-09〜2026-08）取得した。tick取得はDukascopy取得パイプラインで実施し、`batch_pause_ms`の安全域検証（DEC-039・DEC-040、0msへ短縮）・休場スキップ（DEC-038）により、GBPUSD全期間（3,652日、275,854,330 tick）を数時間で取得した。3ペアの追加期間・GBPUSD全期間ともvalidate WARN（エラー0件）、MT5投入照合は全件完全一致、ヒストリー品質100%リアルティックを確認した。CaseFile`mt5/test-config/cases/cross-asset-mfe-check-fx-pairs-full-history.json`（21ケース、`results/backtests/20260924-201756-cases`）。全21ケースで`SIGNAL_ERROR`・`SPREAD_TOO_WIDE`等の異常はなく取引が成立した（NZDUSD-IS-2017のみ取引0件だが、他の近接期間は正常に取引しておりウォームアップ起因の異常ではないと判断）。既存15ケース結果と合算し、4銘柄（AUDUSD/USDCAD/NZDUSD/GBPUSD）でIS（2017）〜Final Holdout相当（2025-2026）の年次推移を測定した。

| 年 | 4銘柄合算 トレード数 | 勝率 | 期待値 | mfe_r | post_peak_mae_r | （参考）FX4銘柄 mfe_r |
|---|---:|---:|---:|---:|---:|---|
| 2017 | 21 | 38.1% | +574円 | 0.859 | -0.718 | — |
| 2018 | 85 | 20.0% | -914円 | 0.777 | -0.903 | 0.725 |
| 2019 | 55 | 34.5% | +465円 | 1.012 | -0.885 | — |
| 2020 | 109 | 27.5% | +235円 | 0.837 | -0.828 | 0.983 |
| 2021 | 88 | 23.9% | -288円 | 0.811 | -0.888 | 0.945 |
| 2022 | 114 | 24.6% | -558円 | 0.720 | -0.836 | 0.931（ピーク） |
| 2023 | 99 | 22.2% | -602円 | 0.814 | -0.964 | 0.917 |
| 2024 | 103 | 34.0% | +847円 | 0.876 | -0.745 | 0.889 |
| 2025-01〜2026-08 | 148 | 26.4% | -538円 | 0.785 | -0.942 | 0.813 |
| **全期間合算** | **822** | **26.6%** | **-189円** | **0.817** | **-0.871** | — |

銘柄別・全期間平均のmfe_rはAUDUSD 0.761・USDCAD 0.776・NZDUSD 0.850・GBPUSD 0.883。

* **2018年の落ち込みが4銘柄合算でも同時に見られる**: mfe_rは2018年に0.777まで落ち込み、FX4銘柄の2018年（0.725）とほぼ同時期・同方向である。前回（2021-2026のみ）の検証では2018年を含んでいなかったため見えていなかった一致であり、「2018年型の落ち込みは循環的・市場全体の現象」という仮説を支持する新たな材料になる。
* **一方、2020年以降の経路はFX4銘柄と明確に異なる**: FX4銘柄は0.983→0.945→0.931（2022ピーク）→0.889→0.813という一貫した単調減少を示すのに対し、4銘柄合算は0.837→0.811→0.720（2022が底）→0.814→0.876→0.785と短周期で上下動し、単調な劣化トレンドは見られない。2022年時点でFX4銘柄がまだピーク圏（0.931）にある一方、4銘柄合算は既に底（0.720）を打って回復し始めている逆方向の動きが見られる。
* **解釈**: 「2018年型は市場全体で循環的に起きるが、2022年以降のFX4銘柄の劣化はそれとは別に、円・ユーロクロス固有の要因が重なっている」という、前回より解像度の高い仮説が立てられる。ただし決定的な証拠ではない。
* **限界**: (1) トレード数が年間十数〜百件程度と少なく、統計的有意性の検定は行っていない。(2) NZDUSD-IS-2017は取引0件で、この年のNZDUSD単体の傾向は評価できない。(3) IS期間（2017-2019）はFold1-5のパラメータ最適化に使われた期間と重複するが、AUDUSD/USDCAD/NZDUSD/GBPUSD自体はパラメータ調整に一切使っていない初見資産であるため、過学習の影響は受けていないと考えられる。(4) 3ペア検証時と同じ限界（tickデータソースの違い、資産固有の価格トレンドとの交絡）も引き続き残る。

**追加分析3の結果（続き）: JPY建て別ペア（AUDJPY/CADJPY）でJPY建て要因とクロス固有要因を切り分け（2026-09-24〜25実施、ユーザー依頼）**: 上記の結果を踏まえ、「2022年以降のFX4銘柄の劣化が円・ユーロクロス固有か、それとも単にJPY建てであることが要因か」を切り分けるため、既存FX4銘柄（USDJPY/EURJPY/GBPJPY、いずれもJPY建て）とは別のJPY建てペア（AUDJPY・CADJPY）を新規に投入した。EURUSD（FX4銘柄中で唯一JPY非建て）がFinal Holdoutで相対的に健闘していたことも、この切り分けの動機である。tickは既存FX4銘柄と同じ2016-09-01〜2026-08-31を取得（AUDJPY 252,615,089 tick、CADJPY 245,752,561 tick、いずれもvalidate WARN・エラー0件・MT5投入照合完全一致・ヒストリー品質100%）。CaseFile`mt5/test-config/cases/cross-asset-mfe-check-jpy-pairs.json`（18ケース、`results/backtests/20260925-063758-cases`）。全18ケースで`SIGNAL_ERROR`等の異常はなく取引が成立した。

| 年 | FX4銘柄（既存） | 円・ユーロ非依存4ペア | AUDJPY/CADJPY（JPY建て別ペア） |
|---|---:|---:|---:|
| 2018 | 0.725 | 0.777 | 0.795 |
| 2020 | 0.983 | 0.837 | 0.877 |
| 2021 | 0.945 | 0.811 | 0.927 |
| 2022 | **0.931（ピーク）** | **0.720（底）** | 0.752 |
| 2023 | 0.917 | 0.814 | **0.714（底）** |
| 2024 | 0.889 | 0.876 | 0.862 |
| FH(25-26) | 0.813 | 0.785 | 0.734 |

（AUDJPY/CADJPY 2銘柄合算、全期間361トレード、勝率28.5%、期待値-617.8円/トレード、mfe_r 0.797。2017年は取引3件のみで参考値扱い）

* **「JPY建てが主要因」ではなく「既存FX4銘柄固有」を支持する結果**: AUDJPY/CADJPYは、FX4銘柄の「2022年ピーク→単調減少」パターンではなく、円・ユーロ非依存4ペアの「上下動」パターンに近い動きを示した。FX4銘柄がまだピーク圏（0.931）にあった2022年、AUDJPY/CADJPYは既に低下局面（0.752）に入り、2023年に底（0.714）を打ってから2024年に回復している。もしJPY建てそのものが2022年以降の劣化の主要因なら、AUDJPY/CADJPYもFX4銘柄と同様の単調減少を示すはずだが、実際には示していない。
...
* [x] **使用する市場データSourceを決定する（2026-09-26確認、既存決定の整理）。** 本番運用ブローカーOANDA証券MT5の正規Web版Tickダウンロードツール（`DECISIONS.md` DEC-023）を主データ源とし、FX10銘柄（USDJPY/EURJPY/EURUSD/GBPJPY/JP225/US30/US100/US500/US2000/XAUUSD）はこれで取得済み。円・ユーロを含まない主要FXペア（AUDUSD/USDCAD/NZDUSD/GBPUSD/AUDJPY/CADJPY、初見資産・探索目的限定）はDukascopy（`DECISIONS.md` DEC-036〜040・DEC-042、ライセンス上のリスクは認識済み）を副データ源とする。
* [x] **データ利用条件とライセンスを確認する（2026-09-26実施、ユーザー依頼）。** OANDA証券Web版Tickダウンロードツール（DEC-023）は本番運用ブローカーの正規口座契約の範囲内であり、追加のライセンス上の懸念はない。**Dukascopy（DEC-036〜040、DEC-042）については、公式サイトのTerms of Useを確認した結果、(1) 自動化ツールでの取得禁止、(2) 非商用利用限定、(3) データベース構築禁止の各条項に、現在の取得・保存方法が文言上抵触する可能性が高いことが判明した。** ユーザーの明示的な判断により、リスクを認識の上で既存データの利用を継続することとした。詳細・留保事項は`DECISIONS.md` DEC-042を参照。
* [x] **TimezoneとDSTの扱いを決定する（2026-09-26確認、既存決定の整理）。** 内部処理はUTCを基準とし、MT5サーバー時刻への変換は`ny_close`ルール（NY現地時刻+7時間、DST自動追従）を使用する（`python/tickdata/timeutil.py`）。OANDA-Japan MT5のサーバー時刻と一致することを実データで実証済み（`docs/tick-data-pipeline.md`、2017-03の週明けtick時刻がDST切替の前後で一貫して月曜00:00台になることを確認）。EA側は確定足のみをEntry判定に使用し（`CLAUDE.md` 6章）、Broker Server Time・UTC・Local Timeを明確に区別する既存実装を維持する。
* [x] **Point-in-time整合性を確認する（2026-09-26実施）。** `mt5/Include/Strategy/TrendFollowingStrategy.mqh`のEntry判定ロジックを確認したところ、`CopyRates(symbol, timeframe, 1, 1, rates)`（shift=1、当該時点で未確定の最新バーshift=0を除く）で確定足のみを参照しており、コード内コメントにも「確定足（shift>=1）のみを参照し、look-ahead biasを発生させない」と明記されている。IS/OOS/Walk Forward/Final Holdoutの時系列分離（`DECISIONS.md` DEC-024/025）とOOS結果を見た後の再最適化禁止の運用（`CLAUDE.md` 9.1）も一貫して守られている。ML学習パイプライン導入時は、Training/Calibration/OOS分離・gapの確保を別途徹底する必要がある（現時点では未着手のため対象外）。
...
* [x] **Data Quality Checkを実装または実行する（2026-09-26確認、既存実装の整理）。** `python/tickdata/validate.py`が全Dukascopyデータセットで実行済み（CROSSED_QUOTE等の異常値棄却、大きなギャップ検出、棄却率チェック、fingerprint一致確認）。OANDA由来tickについては、投入時のtick欠落（0.641%、バッチ末尾128 tickの永続化漏れ）を検出・修正済み（`DECISIONS.md` DEC-037）。両データ源とも、MT5投入後は`VerifyCustomSymbolTicks.mq5`でtick数・時刻の照合、Strategy Testerでの「ヒストリー品質100%リアルティック」確認を全件で実施済み。
...
* [x] **パイプラインを使い、既存FX（EURUSD）をDukascopy由来tickで再構築しOANDA版と一致するか確認した（2026-09-22実施、ユーザー依頼）。** EURUSD 2020-03-01〜03-06（743,000 tick、validate PASS、MT5投入照合一致、ヒストリー品質100%）を、OANDA元データ（サーバー時刻→UTC変換）と1分足単位で突き合わせた。中値の相関係数0.999998、平均乖離0.033 pips（中央値0.05 pips）と極めて高い一致度を確認した。tick頻度はOANDAの方が約6倍高い（中央値557件/分 vs 89件/分）、平均スプレッドはOANDA 0.73 pips・Dukascopy 0.39 pipsで、いずれも別々の流動性プロバイダー由来であることによる自然な差と判断した（tick単位の完全一致は原理的に求めていない）。
* [x] **パイプラインを使い、円・ユーロを含まないFXメジャーペア（AUDUSD/USDCAD/NZDUSD）を新規投入した（2026-09-22〜23実施、ユーザー依頼、`DECISIONS.md` DEC-038〜040）。** 2020-01-01〜2026-08-31（D1 200EMAウォームアップ12か月分を含む）を取得・投入。3銘柄合計376M tick、validate WARN（エラー0件、年末年始等の想定内ギャップとCROSSED_QUOTE棄却率0.001%台のみ）、MT5投入照合は3銘柄とも完全一致、ヒストリー品質100%リアルティックを確認した。取得中に`batch_pause_ms`（Dukascopyへのリクエスト間隔）を3,000ms→500ms→0msへ段階的に短縮する追加検証を実施し採用した（DEC-039・DEC-040）。mfe_r検証結果は2.1.4節「追加分析3の結果（続き）: FXメジャーペア」を参照。
* [x] **AUDUSD/USDCAD/NZDUSDをIS期間相当（2016-09〜）まで遡って追加取得し、GBPUSDを新規追加した（2026-09-23〜24実施、ユーザー依頼）。** 既存FX4銘柄（DEC-023、USDJPY実データ最古日2016-08-31基準）と同じ2016-09-01を開始日とした。AUDUSD/USDCAD/NZDUSDは2016-09-01〜2019-12-31分（各約1,217日）を既存の`*_HIST`へ追記、GBPUSDは2016-09-01〜2026-08-31の全期間（275,854,330 tick）を新規投入した。全4件でvalidate WARN（エラー0件）、MT5投入照合完全一致、ヒストリー品質100%リアルティックを確認した。作業中断時（Windows Update再起動、ジャンクション構成整理のためのユーザーによる一時停止）は`dataset.lock`のstale判定（PID生存確認）をした上で再開し、いずれもデータ破損なく完走した。mfe_r検証結果は2.1.4節「追加分析3の結果（続き）: FXメジャーペアをIS期間相当まで拡張＋GBPUSD新規追加」を参照。
* [x] **JPY建てだが既存FX4銘柄とは別のペア（AUDJPY/CADJPY）を新規追加した（2026-09-24〜25実施、ユーザー依頼）。** 「2022年以降の劣化がJPY建て全般の問題か、既存FX4銘柄固有か」を切り分ける目的。既存FX4銘柄と同じ2016-09-01〜2026-08-31の全期間を新規投入（AUDJPY 252,615,089 tick、CADJPY 245,752,561 tick）。両銘柄ともvalidate WARN（エラー0件）、MT5投入照合完全一致、ヒストリー品質100%リアルティックを確認した。mfe_r検証結果は2.1.4節「追加分析3の結果（続き）: JPY建て別ペア（AUDJPY/CADJPY）でJPY建て要因とクロス固有要因を切り分け」を参照。
...
* [x] **MSCI ACWI指数値の代わりに、代替指標としてeMAXIS Slim全世界株式（オール・カントリー）の基準価額を使用した（2026-09-26実施、ユーザー依頼）。** MSCI ACWI指数自体は契約者向け配信が中心で無料公開されていないため、同指数を追跡する上場投資信託（愛称「オルカン」、三菱UFJアセットマネジメント）の基準価額を代替指標とした。ユーザーが用意した日次基準価額CSV（`D:\.ダウンロード\lst_standard_price_20260926164106.csv`、2018-10-31〜2026-09-25）から、2019-12〜2026-08の81か月分の月末値を抽出し`build/benchmark/benchmark-emaxis-acwi.csv`（`month,level`形式、Git管理外）を作成した。公式サイト（emaxis.am.mufg.jp）で「基準価額は運用管理費用（信託報酬）控除後の値」「分配実績はゼロ（設定来累計0円）」であることを確認し、基準価額そのものが配当込み・信託報酬控除後の実質リターンに相当するため、`--benchmark-annual-fee`は追加控除なし（0）とした（信託報酬の二重控除を回避）。**厳密にはMSCI ACWI指数そのものではなく、これを追跡するファンドの基準価額（トラッキング誤差を含む）である点に注意。**
* [x] **Walk Forward（2020-2024）とFinal Holdout（2025-01〜2026-08）の既存結果でベンチマーク受入基準を判定した（2026-09-26実施）。** `python.analysis.benchmark_comparison`を実データで初めて実行（`results/backtests/20260922-114452-cases`・`20260922-120856-cases`の`trades-normalized.csv`を入力）。**両期間とも不合格（`BENCHMARK_CRITERIA_MET=false`）。**

  | 期間 | EA税引き後CAGR | ベンチマーク税引き後CAGR | EA Sharpe | ベンチマークSharpe |
  |---|---:|---:|---:|---:|
  | Walk Forward（2020-2024） | 2.51% | 15.81% | 0.641 | 1.147 |
  | Final Holdout（2025-2026） | -1.11% | 18.28% | -0.170 | 1.407 |

  この期間（2020-2026）は世界株式市場が非常に好調（ベンチマーク税引き前CAGRはWF 18.73%・FH 22.64%）だったため、EAは絶対収益額としては大幅な悪化ではない場合でも、機会コストの観点では大きく劣後した。**既存のFinal Holdout結果（期待値反転、mfe_r低下）とは独立の、追加のNO-GO材料である。** レポートは`build/benchmark-check/`（Git管理外）。既存のFinal Holdout・Walk Forwardの原因分析（円・ユーロクロス固有の劣化等）とは別軸の問題であり、「循環的か恒久的か」の判別が済んだとしても、この期間の機会コスト面での劣後という事実自体は変わらない。
