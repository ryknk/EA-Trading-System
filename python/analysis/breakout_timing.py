"""Breakout Timing比較分析モジュール。

ブレイクアウトはSetupとTriggerが同一の価格事象（レンジ突破）であり、Entry Timing分析
（`python.analysis.entry_timing`、プルバック専用）が検証する「Trigger成立を待つ」という
概念を適用できない（`docs/backtesting.md`参照）。代わりにEA側`CBreakoutTimingAnalyzer`
（`mt5/Include/Logging/BreakoutTimingAnalyzer.mqh`、`InpEnableBreakoutTimingAnalysis=true`時のみ
動作）が実注文なしでシミュレートした4方式のShadow Trade（IMMEDIATE=ブレイクアウト成立bar自身の
終値で即Entry、CONFIRM_1_BAR/CONFIRM_2_BARS/CONFIRM_3_BARS=1〜3本後もブレイクアウトレベルを
維持できていた場合のみEntry）の監査ログ（`BREAKOUT_TIMING_SETUP`・`BREAKOUT_TIMING_TRADE`イベント）
を集計する。
...
目的はブレイクアウト成立直後の反転（ダマシ）による損失が、即時エントリーというタイミングの問題か、
それとも確認を挟んでも解消しない（Setup/Trigger条件自体の精度の問題）かを切り分ける仮説検証であり、
本モジュールは過去データに最も適合する確認本数を自動選択する処理を一切行わない。
"""
...
from .entry_timing import CHECKPOINT_BAR_OFFSETS, DRAWDOWN_BASELINE_R
from .performance import aggregate_trade_group
from .reports import read_json_lines

VARIANTS = ["IMMEDIATE", "CONFIRM_1_BAR", "CONFIRM_2_BARS", "CONFIRM_3_BARS"]


def _extract_setups(records: list[dict[str, Any]]) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for record in records:
        if record.get("event_type") != "BREAKOUT_TIMING_SETUP":
            continue
        ...
        rows.append({
            ...
            "confirm_1_bar_held": payload.get("confirm_1_bar_held"),
            "confirm_2_bars_held": payload.get("confirm_2_bars_held"),
            "confirm_3_bars_held": payload.get("confirm_3_bars_held"),
        })
    columns = [
        "setup_id", "setup_bar_time", "direction", "pre_entry_mfe_r", "pre_entry_mae_r",
        "confirm_1_bar_held", "confirm_2_bars_held", "confirm_3_bars_held",
    ]
...
def _extract_trades(records: list[dict[str, Any]]) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    for record in records:
        if record.get("event_type") != "BREAKOUT_TIMING_TRADE":
            continue
        payload = record.get("payload")
        if not isinstance(payload, dict):
            continue
...
def variant_summary(trades: pd.DataFrame) -> dict[str, dict[str, Any]]:
    """Variant別（IMMEDIATE/CONFIRM_1_BAR/CONFIRM_2_BARS/CONFIRM_3_BARS）にTrades・Win Rate・
    Profit Factor・Expectancy・Net Profit（R倍数）・Max Drawdown（R倍数、基準値からの下落幅）・
    平均MFE/MAE・価格推移チェックポイント平均を集計する。"""
...
def confirmation_hold_summary(setups: pd.DataFrame) -> dict[str, Any]:
    """ブレイクアウト成立後、1/2/3本後の確定足終値時点でブレイクアウトレベルを維持できていた
    （ダマシで反転していなかった）割合を要約する。維持率が低いほど、直後の反転（ダマシ）が
    頻発していることを示す。"""
    if setups.empty:
        return {
            "setups_observed": 0,
            "confirm_1_bar_held_rate": None,
            "confirm_2_bars_held_rate": None,
            "confirm_3_bars_held_rate": None,
    ...
    return {
        ...
        "confirm_1_bar_held_rate": float(setups["confirm_1_bar_held"].fillna(False).astype(bool).mean()),
        "confirm_2_bars_held_rate": float(setups["confirm_2_bars_held"].fillna(False).astype(bool).mean()),
        "confirm_3_bars_held_rate": float(setups["confirm_3_bars_held"].fillna(False).astype(bool).mean()),
...
def build_breakout_timing_context(paths: list[Path]) -> tuple[pd.DataFrame, pd.DataFrame]:
    """監査JSONLからBREAKOUT_TIMING_SETUP/BREAKOUT_TIMING_TRADEを再構成する。
    `InpEnableBreakoutTimingAnalysis=false`（既定値）で取得した監査ログには対象イベントが存在せず、
    両DataFrameとも空になる。"""
...
def _markdown(variants: dict[str, dict[str, Any]], hold: dict[str, Any]) -> str:
    lines = [
        "# Breakout Timing比較レポート", "",
        "同一のブレイクアウトSetupに対する4方式（即時Entry・1本後/2本後/3本後の維持確認）を",
        "実注文なしのShadow Tradeとして比較した結果です。損益はR倍数（Shadow Trade自身の",
        "当初SL距離を1R）で表しており、口座通貨建てではありません。", "",
        "本レポートは仮説の発見にのみ使用し、確認本数の自動選択・適用は行っていません。", "",
        "## ブレイクアウト成立後の維持率（ダマシの発生頻度）", "",
        f"- 観測したSetup数: {hold['setups_observed']}",
    ]
    for label, key in (
        ("1本後", "confirm_1_bar_held_rate"),
        ("2本後", "confirm_2_bars_held_rate"),
        ("3本後", "confirm_3_bars_held_rate"),
    ):
        rate = hold[key]
        lines.append(f"- {label}も維持できていた割合: {'算出不能' if rate is None else f'{rate:.2%}'}")
    average_mae = hold["average_pre_entry_mae_r"]
    lines.append(f"- 平均逆行(MAE) R: {'算出不能' if average_mae is None else f'{average_mae:.4f}'}")
    average_mfe = hold["average_pre_entry_mfe_r"]
...
def write_report(
    ...
    hold = confirmation_hold_summary(setups)
    report = {
        "schema_version": "1.0",
        ...
        "definitions": {
            ...
            "max_drawdown_r": "pnl_rの累積値に対する最大下落幅。Variant間比較専用の相対指標であり口座資金とは無関係",
            "average_checkpoint_r": "Entry後の固定バー数(1/2/3/5/10/20)時点の価格をR換算した平均値。SL/TP到達等でShadow Tradeが先に終了した場合、到達しなかった本数は集計対象外",
            "pre_entry_mae_r": "ブレイクアウト成立bar終値を基準に、全確認本数解決までの間に想定方向と逆側へ最も動いた距離のR換算",
            "pre_entry_mfe_r": "ブレイクアウト成立bar終値を基準に、全確認本数解決までの間に想定方向へ最も動いた距離のR換算",
            "confirm_n_bar_held_rate": "ブレイクアウト成立からN本後の確定足終値時点でも、Setup成立時点のブレイクアウトレベルを維持できていた（反転=ダマシに遭っていない）割合",
        },
        "confirmation_hold": hold,
        "variants": variants,
    }
    paths = {
        "json": output_directory / "breakout-timing-report.json",
        "markdown": output_directory / "breakout-timing-report.md",
        "setups": output_directory / "breakout-timing-setups.csv",
        "trades": output_directory / "breakout-timing-trades.csv",
    }
    paths["json"].write_text(
        json.dumps(report, ensure_ascii=False, indent=2, allow_nan=False) + "\n", encoding="utf-8",
    )
    paths["markdown"].write_text(_markdown(variants, hold), encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Breakout Timing比較分析（IMMEDIATE/CONFIRM_1_BAR/CONFIRM_2_BARS/CONFIRM_3_BARS）")
    parser.add_argument("--input", type=Path, action="append", required=True, help="監査JSONL。複数指定可")
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args(argv)
    setups, trades = build_breakout_timing_context(args.input)
